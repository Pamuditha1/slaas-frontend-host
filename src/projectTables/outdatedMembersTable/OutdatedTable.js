...
export const OutdatedTable = (props) => {
  ...
  useEffect(() => {
    async function fetchOutdated() {
      ...
      let notTerminated = members.filter((m) => {
        if (m.status == "Member") return true;
      });
            ...
            <div className="col-6">
              <Link to="/user/outdated-list/send-emails">
                <Button
                  ...
                  color="dark"
                >
                  Send Emails to {selectedFlatRows.length}
                </Button>
              </Link>
            </div>
            <div className="col-6">
                  ...
                  <tr {...row.getRowProps()}>
                    {row.cells.map((cell) => {
                      return (
                        <td
                          {...cell.getCellProps()}
                          style={{ color: cell.value == "Terminated" && "red" }}
                        >
                          {cell.column.Header ==
                            "Last Membership Payment Date" ||
                          cell.column.Header == "DOT" ||
                          cell.column.Header == "Last Payment Date"
                            ? cell.value
